...
const weatherService = {
      ...
      wx.request({
        ...
        success: (res) => {
          ...
          if (res.statusCode === 200) {
            ...
            const errorMsg = `获取天气数据失败: ${res.statusCode}, ${JSON.stringify(res.data)}`;
   ...
   * @returns {Promise} 返回天气数据Promise
   */
  getCurrentWeatherByCoords: function(lat, lon, lang = 'zh_cn', units = 'metric') {
    return new Promise((resolve, reject) => {
      console.log(`开始获取坐标天气: 纬度=${lat}, 经度=${lon}, 语言=${lang}, 单位=${units}`);
      wx.request({
        url: `${BASE_URL}/weather`,
        data: {
          lat: lat,
  ...
  formatForecastData: function(data) {
    ...
    forecast.forEach(item => {
      ...
      if (!dailyMap[dateStr] || (hour >= 11 && hour <= 14)) {
        ...
        // 更新最高温和最低温
        if (item.main) {
          if (!dailyMap[dateStr].main.temp_max || item.main.temp_max > dailyMap[dateStr].main.temp_max) {
            dailyMap[dateStr].main.temp_max = item.main.temp_max;
          }
          if (!dailyMap[dateStr].main.temp_min || item.main.temp_min < dailyMap[dateStr].main.temp_min) {
    ...
    const daily = Object.values(dailyMap).map(item => {
      ...
      return {
        ...
        weatherIcon: `https://openweathermap.org/img/wn/${weather.icon || '01d'}@2x.png`,
