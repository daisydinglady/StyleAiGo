...
Page({
  ...
  filterItems: function () {
    ...
    if (this.data.filters.colors.length > 0) {
      items = items.filter((item) =>
        this.data.filters.colors.includes(item.color)
      );
    }
    ...
    if (this.data.filters.seasons.length > 0) {
      items = items.filter((item) => {
        return item.seasons.some((season) =>
          this.data.filters.seasons.includes(season)
        );
      });
    }
  ...
  addItem: function () {
    ...
    // 更新本地存储
    this.saveWardrobeItems(wardrobeItems);
  ...
  // 加载衣物列表
  loadWardrobeItems: function () {
    const wardrobeItems = wx.getStorageSync("wardrobeItems") || [];
    this.setData({
  ...
  // 保存衣物列表到本地存储
  saveWardrobeItems: function (items) {
    wx.setStorageSync("wardrobeItems", items);
  },
});
